  ...
  return (
    <>
      <section className='relative aspect-square md:aspect-video'>
        ...
        <div className='from-background dark:from-dark absolute bottom-0 left-0 flex h-full w-full flex-col justify-end bg-linear-to-t to-transparent p-4'>
          <div className='mx-auto max-w-4xl'>
            <p className='text-sm'>{formatDate(post.updatedAt)}</p>
            <h1 className={title()}>{post.title}</h1>
            <div className='my-4 rounded-lg italic'>{post.summary}</div>

            <Button color='primary' startContent={<FaArrowLeft />} href='/blog' as={Link} aria-label='Volver'>
              Volver
            </Button>
          </div>
        </div>
      </section>
      <section className='prose dark:prose-invert mx-auto max-w-4xl p-6'>
