...
import { AiFillAlert, AiFillRightCircle, AiOutlineClose, AiOutlineSearch } from 'react-icons/ai';
...
export const Cmdk = () => {
  ...
  const CloseButton = useCallback(
    ({ onPress, className }: { onPress?: ButtonProps['onPress']; className?: ButtonProps['className'] }) => {
      return (
        <Button
          isIconOnly
          className={clsx('data-[hover=true]:bg-content1 border-default-400 dark:border-default-100 border', className)}
          radius='full'
          size='sm'
          variant='bordered'
          onPress={onPress}
  ...
  const renderItem = useCallback(
    (item: SearchResultItem, index: number, isRecent = false) => {
      ...
      const mainIcon = isRecent ? (
        <AiOutlineSearch className={slots.leftIcon()} size={20} strokeWidth={2} />
      ) : isLvl1 ? (
  ...
  return (
    ...
    >
      <ModalContent>
        <Command className={slots.base()} label='Comando de búsqueda rápida' shouldFilter={false}>
              ...
              <Command.Empty>
                <div className={slots.emptyWrapper()}>
                  <div>
                    ...
                    {query.length === 1 ? (
                      <p className=''>Intente agregar más caracteres a su término de búsqueda.</p>
                    ) : (
