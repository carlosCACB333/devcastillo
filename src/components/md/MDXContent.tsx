...
const LinkedHeading: React.FC<LinkedHeadingProps> = ({ as, linked = true, id: idProp, className, children }) => {
  const Component = as;

  const id = idProp || virtualAnchorEncode(children as string);

  return (
    <Component className={clsx(!linked && className)} data-id={id} data-name={children} id={id}>
  ...
  return (
    <Markdown
      ...
      components={{
        ...
        pre: ({ children, className }) => <pre className={clsx('not-prose', className)}>{children}</pre>,

        code: ({ children, className }) => <CodeElement className={className}>{children}</CodeElement>,
        ...
        blockquote: ({ ...props }) => <Blockquote color={'primary' as any} {...props} />,
      }}
    >
      {children}
    </Markdown>
